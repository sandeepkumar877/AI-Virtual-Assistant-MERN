...
`;
    ...
    const textResponse = chatCompletion.choices?.[0]?.message?.content || null;

    console.log("Groq API raw result:", textResponse);

    return textResponse;
  } catch (error) {
    console.log("Error fetching Groq response:", error);
  }
};
